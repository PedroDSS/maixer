...
import {
  IngredientSelector,
  Ingredient,
} from "@/components/IngredientSelector";
...
export default function NewRecipePage() {
  ...
  async function onSubmit(data: RecipeGenerationFormValues) {
    try {
      ...
      // Validate that we have ingredients based on the active tab
      if (
        activeTab === "selector" &&
  ...
  return (
    <div className="container max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-grow flex flex-col">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold mb-2 bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
            Nouvelle Recette
          </h1>
      ...
      <div className="flex-grow flex items-center justify-center pb-8">
        <Card className="w-full max-w-2xl backdrop-blur-sm bg-white/90 shadow-lg border-0 ring-1 ring-orange-200">
          <CardHeader className="text-center space-y-4 pb-6 sm:pb-8">
            ...
            <div>
              ...
              <CardDescription className="text-gray-600 mt-2">
                Notre IA créera une recette personnalisée basée sur vos
                ingrédients et préférences.
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              ...
              >
                <Tabs
                  ...
                  value={activeTab}
                  onValueChange={(value) =>
                    setActiveTab(value as "selector" | "manual")
                  }
                  className="w-full"
                >
                  <TabsList className="grid grid-cols-2 mb-4">
                    <TabsTrigger value="selector">
                      Ingrédients Airtable
                    </TabsTrigger>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription className="text-gray-500 text-xs sm:text-sm">
                            Sélectionnez les ingrédients que vous souhaitez
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription className="text-gray-500 text-xs sm:text-sm">
                            Listez tous les ingrédients que vous souhaitez
                            utiliser dans la recette.
                          </FormDescription>
                          <FormMessage className="text-red-500" />
                        </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-gray-700">
                        Allergies ou Intolérances Alimentaires (Optionnel)
                      </FormLabel>
                      <FormControl>
                        <Textarea
                      ...
                      <FormDescription className="text-gray-500 text-xs sm:text-sm">
                        Listez les allergies ou intolérances à éviter dans la
                        recette.
                      </FormDescription>
                      <FormMessage className="text-red-500" />
                    </FormItem>
          ...
          <CardFooter className="text-xs sm:text-sm text-gray-500 text-center px-4 py-4 sm:py-6">
            Astuce : Plus votre liste d'ingrédients est précise, meilleure sera
            la recette.
          </CardFooter>
        </Card>
      </div>
